...
class Net(torch.nn.Module):
    def __init__(self, T):
        ...
        thr = [0.2, 1]  # The threshold of hidden and output neurons
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            if (batch_idx+1) % 10 == 0:
                print('Test [{}/{} ({:.0f}%)]\tLoss: {:.6f} Accuracy: {:.4f}%'.format(
                    (batch_idx+1) * len(data), len(test_loader.dataset),
                    100. * (batch_idx+1) / len(test_loader), test_loss/(batch_idx+1), 100. * correct/(batch_idx+1)/len(data)), end='\r')
        print()
def main():
    # Training settings
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        scheduler.step()
        if args.save_model:
            torch.save(model.state_dict(), "mnist_mlp_ttfs2_receive.pt")
    test(model, device, test_loader)

    if args.epochs != 0 and args.save_model:
        torch.save(model.state_dict(), "mnist_mlp_ttfs2_final_receive.pt")
