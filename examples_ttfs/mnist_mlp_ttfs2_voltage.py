...
class Net(torch.nn.Module):
    def __init__(self, T):
        ...
        self.fc1 = ttfsCat.Dense_Voltage(28*28, 400, T=T, theta=thr[0])
        self.fc2 = ttfsCat.Dense_Voltage(400, 10, T=T, theta=thr[1])
        self.T = T
        '''
        np.random.seed(0)
        with torch.no_grad():
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    tmax = torch.tensor(model.T, device=device, dtype=torch.float32)

    for batch_idx, (data, target) in enumerate(train_loader):
        data, target = data.to(device), target.to(device)
        output,voltage = model((1.0-data.flatten(start_dim=1))*(model.T-1))
        loss = ttfsCat.relative_loss.apply(output, target, tmax, 5).mean()
        total_loss += loss
        minFire,pred = output.min(dim=-1, keepdim=True)
        #pred = output.argmin(dim=-1, keepdim=True)  # get the index of the max log-probability
        pred2 = voltage[-3].argmax(dim=-1, keepdim=True)
        pred = torch.where(minFire<tmax, pred, pred2)
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            data, target = data.to(device), target.to(device)
            output, voltage = model((1.0-data.flatten(start_dim=1))*(model.T-1))
            test_loss +=  ttfsCat.relative_loss.apply(output, target, tmax, 5).mean()
            minFire,pred = output.min(dim=-1, keepdim=True)
            #pred = output.argmin(dim=-1, keepdim=True)  # get the index of the max log-probability
            pred2 = voltage[-3].argmax(dim=-1, keepdim=True)
            pred = torch.where(minFire<tmax, pred, pred2)
...
def main():
    ...
    dataset2 =  datasets.MNIST('../data', train=False,
                       transform=transform)
    dataset1k = Subset(dataset2, range(1000))
    ...
    if args.resume != None:
        ...
        ttfsCat.transfer(model, args.resume)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #scheduler.step()
        if args.save_model and best < correct:
            torch.save(model.state_dict(), "mnist_mlp_ttfs2_best.pt")
            best = correct
    if args.epochs == 0:
        test(model, device, test_loader)

    if args.epochs != 0 and args.save_model:
        torch.save(model.state_dict(), "mnist_mlp_ttfs2_final.pt")
