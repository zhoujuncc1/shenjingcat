...
def main():
    ...
    parser.add_argument('--weight-quantize', type=int, default=32)
    ...
    model = MLP(mlp_config, activation_quantize_factor=-1, clamp_max = 1.0, bias=False, weight_quantize_bit=args.weight_quantize, dropout=0, activation=args.activation).to(device)
    ...
    with torch.no_grad():
        normalize_weight(snn_model.mlp, quantize_bit=args.weight_quantize)
